...
n = 0
total = np.sum([len(list(Path(k).glob("*/*/"))) for k in args.input_dir.split(",")])
            ...
            with Image() as big_montage_img:
                ...
                if len(states) == 0:
                    ...
                    with Image() as montage_img:
                        for i in range(
                            len(list(variation.glob(f"*.{args.input_extension}")))
                        ):
                            imagepath = Path(variation, f"{i}.{args.input_extension}")
                        ...
                        montage_img.border(color=args.border_color, width=5, height=5)
                        # Write label.
                        montage_img.background_color = args.label_background_color
                        montage_img.splice(
                            x=0,
                            y=0,
                            width=0,
                        ...
                        font_size = montage_img.width // len(variation.name)
                        with Drawing() as ctx:
